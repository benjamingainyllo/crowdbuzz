-- ============================================================
-- CrowdBuzz — complete database setup
--
...
-- PART 4 — Getting paid
--
-- CrowdBuzz NEVER holds creator money. The payment provider splits
...
DROP POLICY IF EXISTS "CrowdBuzz public read" ON storage.objects;
CREATE POLICY "CrowdBuzz public read" ON storage.objects
...
DROP POLICY IF EXISTS "CrowdBuzz authenticated upload" ON storage.objects;
CREATE POLICY "CrowdBuzz authenticated upload" ON storage.objects
...
DROP POLICY IF EXISTS "CrowdBuzz authenticated update" ON storage.objects;
CREATE POLICY "CrowdBuzz authenticated update" ON storage.objects
  FOR UPDATE USING (
    bucket_id IN ('avatars', 'event_covers', 'offer_covers')
    AND auth.role() = 'authenticated'
  );
...
CREATE TABLE IF NOT EXISTS public.event_cohosts (
  ...
  -- Optional link to a CrowdBuzz handle, so a cohost who is also an
...
-- CrowdBuzz needs no account, so saving one must not either — asking a
