...
export async function refundOrder(orderId: string, amountKobo: number, reason: string) {
  try {
    ...
    // The dangerous half is shared with the organiser's own refund button
    // in lib/refunds.ts — written twice, the second copy is always the one
    // that forgets the over-refund guard. What stays here is the audit
    // trail, which is the part that is genuinely different for an admin.
    ...
    if (!result.before) return fail(new Error(result.error ?? "Could not start that refund."));
    ...
    await recordAdminAction({
      ...
      subjectType: "order",
      subjectId: orderId,
      subjectLabel: result.reference ?? orderId,
      previousValue: {
        status: result.before.status,
        already_refunded_kobo: result.before.alreadyRefundedKobo,
      },
      newValue: {
        amount_kobo: result.amountKobo,
        platform_fee_returned_kobo: result.feeReturnedKobo,
