 ...
 * Starts a checkout.
 *
 * CrowdBuzz never receives the buyer's money. For paid items the provider
...
export async function createCheckoutSession(payload: CheckoutPayload): Promise<CheckoutResult> {
  ...
  try {
    ...
    if (
      ...
      // the money in CrowdBuzz's account, which we must never do.
      await releaseProducts(productsTaken);
      await releaseReservation(reservation);
      return {
        success: false,
    ...
    // the organiser spent their "next event free" on this one, so CrowdBuzz
    ...
    // WHO PAYS WHAT.
    //
    //   CrowdBuzz's fee  -> the organiser, out of what they set
    ...
    const { data: paidOrder, error: insertError } = await admin
      ...
      .insert({
        ...
        // two is CrowdBuzz's to absorb, by design.
