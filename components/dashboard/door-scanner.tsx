    ...
    <div className="mx-auto w-full max-w-md space-y-4 pb-24">
      <header className="flex items-center gap-3">
        <Link
          ...
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[8px] border border-[var(--dl-line)] text-subtle transition-colors hover:bg-muted hover:text-text"
        >
      ...
      <div className="flex gap-2">
        {(["camera", "manual"] as const).map((m) => (
          <button
            key={m}
            onClick={() => setMode(m)}
            className={`flex flex-1 items-center justify-center gap-2 rounded-[8px] border py-2.5 text-xs font-bold transition-colors ${
              mode === m
      ...
      {mode === "camera" ? (
        <div className="overflow-hidden rounded-[8px] border border-[var(--dl-line)] bg-black">
          <div className="relative aspect-square w-full">
            ...
            {cameraState === "live" && (
              <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                <div className="h-48 w-48 rounded-[8px] border-4 border-white/70 shadow-[0_0_0_9999px_rgba(0,0,0,0.45)]" />
              </div>
            )}
                  ...
                  <>
                    ...
                    <div className="flex gap-2">
                      {cameraState === "denied" && (
                        <button
                          onClick={startCamera}
                          className="rounded-[8px] bg-white px-3 py-2 text-xs font-bold text-black"
                        >
                          Try again
                        </button>
                      )}
                      <button
                        ...
                        className="rounded-[8px] border border-[var(--dl-line)] px-3 py-2 text-xs font-bold text-white"
                      >
                        Type it instead
                      </button>
                    </div>
        ...
        <form onSubmit={handleManualSubmit} className="space-y-3">
          ...
          <button
            ...
            className="flex h-12 w-full items-center justify-center gap-2 rounded-[8px] bg-text text-sm font-bold text-background transition-opacity hover:opacity-90 disabled:opacity-40"
          >
...
function ResultBanner({
  ...
  return (
    <div className={`rounded-[8px] ${tone.bg} ${tone.text} p-5`}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {result.collect && result.collect.length > 0 && (
            <div className="mt-3 rounded-[8px] border border-black/25 bg-black/15 p-3">
                ...
                {result.collect.map((line) => (
                  <li key={line.id} className="flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-[8px] bg-black/25 px-3 py-1.5 text-[11px] font-black uppercase tracking-wide transition-colors hover:bg-black/40 disabled:opacity-50"
                    >
      ...
      <div className="mt-4 flex gap-2">
        <button
          onClick={onDismiss}
          className="flex-1 rounded-[8px] bg-black/15 py-2.5 text-xs font-bold transition-colors hover:bg-black/25"
        >
          Next
        </button>
        {result.outcome === "admitted" && (
          <button
            onClick={onUndo}
            className="flex items-center justify-center gap-1.5 rounded-[8px] bg-black/15 px-4 py-2.5 text-xs font-bold transition-colors hover:bg-black/25"
          >
