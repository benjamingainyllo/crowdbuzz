...
export function ComingSoon({
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="rounded-[8px] border border-[var(--dl-line)] bg-surface p-8 sm:p-10">
        <div className="flex h-12 w-12 items-center justify-center rounded-[8px] bg-muted text-subtle">
          <Icon className="h-6 w-6" />
        </div>

        <div className="mt-6 flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-[8px] border border-[#FFDE5940] bg-[#FFDE591a] px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[var(--marker)]">
          ...
          <ul className="mt-3 space-y-2">
            {planned.map((item) => (
              <li key={item} className="flex items-start gap-2.5 text-sm text-subtle">
                <span className="mt-[7px] h-1 w-1 shrink-0 rounded-[8px] bg-subtle/50" />
                {item}
              </li>
            ))}
          </ul>
        ...
        <Link
          ...
          className="mt-8 inline-flex h-10 items-center justify-center rounded-[8px] bg-text px-5 text-xs font-bold text-background transition-transform hover:scale-[1.02]"
        >
          {insteadLabel}
        </Link>
      </div>
